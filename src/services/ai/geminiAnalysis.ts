  ...
  async analyzeIssue(input: AIAnalysisInput): Promise<AIAnalysisResult> {
    ...
    for (const modelName of MODELS) {
      ...
      try {
        ...
        // Sanitise the AI output through the same validator we test in
        // isolation — see professionValidation.test.ts for the edge cases.
        const professions = validateProfessionKeys(parsed.professions);
