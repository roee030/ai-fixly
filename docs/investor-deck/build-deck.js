...
// ═════════════════════════════════════════════════════════════════════════
{
  const s = pres.addSlide();
  lightBg(s);
  addTitle(s, 'מי עוד בשוק?', 'המתחרים האמיתיים בישראל — ואיפה אנחנו עוקפים אותם.');

  // שלושה מתחרים ישראליים מרכזיים: מדרג (פלטפורמה עם דירוגים),
  // המקצוענים (אינדקס בעלי מקצוע) ודפי זהב (ערוץ הוותיק). כל השאר
  // נופלים תחת אחד מהשלושה.
  const header = ['', 'ai-fixly', 'מדרג', 'המקצוענים', 'דפי זהב'];
  const body = [
    ['חיכוך הלקוח',       'לחיצה אחת',   'טופס + שיחות',      'טופס + שיחות',    'רולטת טלפון'],
    ['Onboarding לבעל מקצוע','שום דבר',  'הרשמה + אימות',      'פרופיל באתר',     'מודעה בתשלום'],
    ['סיווג AI',           'כן',          'לא',                 'לא',               'לא'],
    ['הצעות אנונימיות',     'כן',          'לא — שם מיד',        'לא — שם מיד',      'לא'],
    ['זמן להצעה ראשונה',    'פחות מ-5 דק׳','שעות',              'שעות',             'שעות/ימים'],
    ['המתווך בשיחה',        'אנחנו (AI)',  'הלקוח לבד',          'הלקוח לבד',        'הלקוח לבד'],
  ];
    ...
    ...body.map((row, ri) =>
      row.map((cell, ci) => ({
        ...
        options: {
          ...
          bold: ci === 1 || ci === 0,
          fontSize: 10.5, fontFace: ci === 0 ? FONT_HEAD : FONT_BODY,
...
// אין שקף סיכום/בקשה — זאת מצגת מוצר ("מה בנינו, איך זה עובד"),
// לא מצגת גיוס. טראקשן סוגר את הסיפור.
